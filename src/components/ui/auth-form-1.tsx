...
const signUpSchema = z.object({
  ...
  email: z.string().email("Invalid email address"),
  password: z.string().min(8, "Password must be at least 8 characters"),
  // terms stored as boolean, but we enforce true at validation time
  terms: z
    .boolean()
...
interface AuthFormProps {
  onSubmit: React.FormEventHandler<HTMLFormElement> | ((...args: any[]) => any);
...
function AuthForm({ onSubmit, children, className }: AuthFormProps) {
  return (
    <form data-slot="auth-form" className={cn("space-y-6", className)} onSubmit={onSubmit}>
      {children}
    </form>
  );
}
...
function AuthSignIn({ onForgotPassword, onSignUp }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    try {
      const ok = await auth.signin(data.email, data.password);
      if (ok) {
        navigate('/dashboard');
        return;
      }
...
function AuthSignUp({ onSignIn }: AuthSignUpProps) {
  ...
  const onSubmit = async (data: SignUpFormValues) => {
    setFormState((prev) => ({ ...prev, isLoading: true, error: null }));
    try {
      // For now, create account via auth.signin to keep flow simple
      const ok = await auth.signin(data.email, data.password);
      if (ok) {
        navigate('/dashboard');
        return;
